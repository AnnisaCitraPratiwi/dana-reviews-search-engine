            ...
            try {
                ...
                if (data.success && data.total_results > 0) {
                    ...
                    summaryDiv.innerHTML = `
                        ...
                        Ditemukan <strong>${data.total_results}</strong> review yang cocok dengan pencarian "<strong>${data.query}</strong>"
                    `;
                    ...
                    data.results.forEach(review => {
                        ...
                        card.innerHTML = `
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <div>
                                    <h5 class="fw-bold mb-1">${escapeHtml(review.user)}</h5>
                                    <div class="rating-stars mb-2">${stars} (${review.rating}/5)</div>
                                </div>
                                <span class="similarity-badge">${(review.similarity_score * 100).toFixed(0)}% Match</span>
                            </div>
                            <p class="mb-2">${escapeHtml(review.review_text)}</p>
                            ...
                            ${review.reply_content ? `
                                <div class="reply-section">
                                    <div class="reply-label">
                                        <i class="fas fa-reply me-1"></i>Balasan dari DANA:
                                    </div>
                                    <p class="mb-0 small">${escapeHtml(review.reply_content)}</p>
                                </div>
                            ` : ''}
                        `;
                ...
                } else if (data.success && data.total_results === 0) {
                    resultsContainer.innerHTML = `
                        <div class="no-results">
                            <i class="fas fa-search fa-3x mb-3 text-muted"></i>
                            <h5>Tidak ditemukan review yang cocok</h5>
                    ...
                    resultsContainer.innerHTML = `
                        <div class="no-results text-danger">
                            ...
                            <h5>Pencarian gagal</h5>
                            <p>${data.message || 'Terjadi kesalahan saat mencari data'}</p>
                        </div>
                    `;
                }
